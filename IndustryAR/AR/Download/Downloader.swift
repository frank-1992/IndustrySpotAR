...
typealias DownloadCompletionClosure = ((_ filePath: String, _ fileName: String)->Void)
...
class Downloader: NSObject {
    ...
    private var suggestFileName: String = "unKnownFileTitle.tmp"
...
extension Downloader: URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        //初始化本地文件地址
        // 远程文件名称
        suggestFileName = dataTask.response?.suggestedFilename ?? "unKnownFileTitle.tmp"
        let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        localUrl = dir.appendingPathComponent("Download/" + suggestFileName)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil {
            ...
            self.completion?(self.localUrl?.absoluteString ?? "", self.suggestFileName)
        }
    }
}
